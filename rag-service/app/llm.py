"""LLM dispatcher using Groq (free, fast Llama 3.3 70B)."""
...
SYSTEM_PROMPT = (
    ...
    "Use the provided context to answer questions about pet health, nutrition, and care. "
    "If the context does not contain enough information, say so and recommend consulting a veterinarian. "
...
def _rule_based(question: str, pet_context: Optional[dict]) -> str:
    ...
    if any(w in q for w in ["food", "diet", "eat", "treat"]):
        return (f"{who}Match diet to species, life stage, and weight. "
                "Avoid chocolate, grapes/raisins, onions, garlic, xylitol for dogs. "
                "Cats should not have lilies, onions, or excessive raw fish. Consult your vet.")
    if "vaccin" in q:
    ...
    if any(w in q for w in ["flea", "tick", "worm"]):
        return (f"{who}Year-round parasite prevention is recommended. "
                "Common options include Bravecto, NexGard, Frontline, and Heartgard.")
...
async def generate_answer(
    ...
    # Try Groq first (free, fast, cloud)
